...
class HeartFailureModels:
    ...
    def train_logistic_regression(self, C=1.0):
        ...
        print(f"Logistic Regression - Accuracy: {self.results['Logistic Regression']['accuracy']:.4f} - Tiempo: {training_time:.2f}s")
        return lr
    ...
    def train_gradient_boosting(self, n_estimators=100, learning_rate=0.1):
        ...
        self.results['Gradient Boosting']['training_time'] = training_time 
        
        print(f"Gradient Boosting - Accuracy: {self.results['Gradient Boosting']['accuracy']:.4f} - Tiempo: {training_time:.2f}s")
        return gb
    ...
    def train_svm(self, C=1.0, kernel='rbf'):
        ...
        print(f" SVM - Accuracy: {self.results['SVM']['accuracy']:.4f} - Tiempo: {training_time:.2f}s")
        return svm
    ...
    def predict_single_patient(self, patient_data, model_name='Random Forest'):
        ...
        if model_name == 'Deep Learning':
            proba = float(model.predict(patient_scaled, verbose=0).flatten()[0])
            prediction = int(proba > 0.5)
        else:
            prediction = int(model.predict(patient_scaled)[0])
            proba = float(model.predict_proba(patient_scaled)[0, 1])
